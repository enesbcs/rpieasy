# Original author: tatobari https://github.com/tatobari/hx711py
# Adapted to RPIEasy gpios
import gpios
from time import sleep

class HX711:

    def __init__(self, dout, pd_sck, gain=128): #dout: input, sck: output
        self.DOUT = dout
        self.PD_SCK = pd_sck
    ...
    def is_ready(self):
        return gpios.HWPorts.input(self.DOUT) == 0
    ...
    def set_gain(self, gain):
        ...
        gpios.HWPorts.output(self.PD_SCK, False)
    ...
    def readNextBit(self):
       ...
       gpios.HWPorts.output(self.PD_SCK, True)
       gpios.HWPorts.output(self.PD_SCK, False)
       value = gpios.HWPorts.input(self.DOUT)
    ...
    def read_median(self, times=3):
       ...
       if (times & 0x1) == 0x1:
          ...
          return sum(valueList[midpoint-1:midpoint+2]) / 2.0
