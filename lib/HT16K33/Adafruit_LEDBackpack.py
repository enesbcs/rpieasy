...
from lib.HT16K33.Adafruit_I2C import Adafruit_I2C
...
class LEDBackpack:
  ...
  # Constructor
  def __init__(self, address=0x70, i2cbusnum=-1, debug=False):
  ...
  def clear(self, update=True):
    ...
    if (update):
      self.writeDisplay()
try:
 if len(ht16_devices)<=0:
  ht16_devices = []
except:
  ht16_devices = []

def request_ht16_devices(i2ca,i2cb):
 global ht16_devices
 for i in range(len(ht16_devices)):
  if (ht16_devices[i].i2cbn == int(i2cb)) and (ht16_devices[i].address == int(i2ca)):
   return ht16_devices[i]
 ht16_devices.append(LEDBackpack(address=i2ca,i2cbusnum=i2cb))
 return ht16_devices[-1]
